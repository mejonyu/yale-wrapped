...
var sp2tx = {
  ...
  start: function() {
    ...
    sp2tx.socket.onmessage = function (event) {
      var user_speech = event.data;
      if (user_speech !== "") {
          console.log("User speech input received: "+user_speech);
        //   recommendSong(user_speech);
      }
    }
  }
};
...
// To be implemented in results page
...
// async function recommendSong(user_speech){
//   const prompt = "Recommend one song available on Spotify that evokes a similar mood and/or has a similar genre and style to the song: "+user_speech+"." +
...
//                   "If "+user_speech+" is not a song, please ask the user to try again with the name of a song."
